...
| `sortOrder` | Display order (API sort: `sortOrder`)                          |
...
3. Pass `options: { createdByField: "createdBy" }` to `getPermissionAccess`
...
3. **`admin.components`** — deep merge: default `Field` is kept unless you override the same component key
