...
**Auth users slug:** `config.admin.user` (default `"users"`) — hierarchy (`parent` / `parentPath`), users access, and `options.usersCollectionSlug`. Plugin `createdBy` fields are hidden **`text`** (audit); users `parent` is a **relationship**.
...
| `createdBy`    | Hidden `text` (audit; data scope uses `createdByField: "id"`) |

`parent` / `parentPath` hooks validate cycles and keep descendant paths in sync.

**Data scope on users** uses `createdByField: "id"`, not the `createdBy` audit field.

> **Users access:** use document **`id`** as ownership key — not `username`, `code`, or `email`. App collections: store user **id** in `createdBy` (`text` or `relationship`).
...
The plugin resolves slug from `admin.user`. Seed `permission-features` with `code: "members"`. Pass `usersCollectionSlug: "members"` in `getPermissionAccess` for hierarchy when needed.
