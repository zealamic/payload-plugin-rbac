...
**Auth users slug:** the plugin reads `config.admin.user` (default `"users"`) and passes it to every RBAC collection as `usersCollectionSlug` — used for `createdBy` relationship `relationTo` and for `getPermissionAccess` `options.usersCollectionSlug`. No extra plugin config is required when you change `admin.user`.
...
| `createdBy` | Hidden — auto-set on create to `req.user.id` (audit metadata; access remains super-admin only) |
...
**Reorder API:** `POST /api/permission-actions/reorder` with body `{ sortedItems: [{ id, sortOrder }] }`. Requires an authenticated user with **update** access on the collection.
...
| `status`    | `active` / `inactive` — only `active` rows appear in matrix                         |
| `createdBy` | Hidden — auto-set on create to `req.user.id`                                        |
...
**Reorder API:** `POST /api/permission-features/reorder` — same body shape as permission-actions.
...
| `createdBy`         | Hidden — auto-set on create            |
...
| ----------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| `own`       | Only records owned by the current user (via the configured ownership field — default `createdBy`)                                  |
...
**What you need in your app collections** (Payload does **not** add an ownership field for you):

1. Add a **relationship** to your auth users collection on each collection you protect with `dataScope` (commonly named `createdBy`)
2. Set it on create — usually a `beforeChange` hook (`req.user.id`)
3. Wire `getPermissionAccess`:
   - `read` → `options: {}` when the field is `createdBy`, or `options: { createdByField: "author" }` for a custom name
   - `update` / `delete` → `mode: "modify"`; omit `options` when the field is `createdBy`
   - Custom auth slug → `options: { usersCollectionSlug: "members" }` (defaults to `config.admin.user`)

```ts
// posts — ownership field + access (see dev/collections/posts.ts)
fields: [
  {
    name: "createdBy",
    type: "relationship",
    relationTo: "users", // or your admin.user slug
    admin: { readOnly: true },
  },
],
hooks: {
...
See [UTILS — Ownership field](https://github.com/zealamic/payload-plugin-rbac/blob/main/docs/UTILS.md#ownership-field-required-for-data-scope) for the full `getPermissionAccess` reference and a custom field name example (`author`).
...
| `createdBy`    | Hidden — auto-set on create (audit; **not** used for data scope — see below) |

`parent` / `parentPath` hooks validate cycles and keep descendant paths in sync. A `beforeChange` hook also sets `createdBy` on user create.

**Data scope on users** uses `createdByField: "id"` (each user document is “owned” by itself), not the `createdBy` relationship field.

> **Warning — use document `id`, not `username` / `code` / custom keys**
>
> On the **users** collection, row-level access is tied to Payload’s document **`id`** (the primary key). Do **not** point `createdByField` at `username`, `code`, `email`, or any other business field for users access.
>
> - **Why:** `id` is stable, unique, and what relationships (`createdBy`, `parent`, hierarchy lookups) store. Usernames and codes can change, collide, or be reused — scope checks would break or leak access.
> - **If you add `username`, `code`, etc.:** keep them **`unique`** for login or display, but treat them as normal data fields — they do **not** replace `id` as the ownership key for RBAC on users.
> - **App collections** (`posts`, …): ownership fields should still be a **relationship to users** storing the user’s **`id`**, not their username.
...
| `update`  | `update`     | `mode: "modify"` + data scope (`collectionSlug` defaults to users slug) |
...
The plugin resolves `usersCollectionSlug` from `admin.user` automatically — RBAC `createdBy` fields, hierarchy lookups, and default users access all use `"members"`. Seed `permission-features` with `code: "members"` to match the slug. In app collections, set `relationTo: "members"` on ownership fields and pass `usersCollectionSlug: "members"` in `getPermissionAccess` options when needed.
...
Full example: `dev/collections/posts.ts` — add `createdBy` relationship + create hook, then `getPermissionAccess` for CRUD. See [UTILS](https://github.com/zealamic/payload-plugin-rbac/blob/main/docs/UTILS.md#getpermissionaccessargs) for `author` / custom `createdByField` patterns.

**Exported helpers** (optional reuse in your collections):

- `getCreatedByRelationshipField(usersCollectionSlug)` — hidden `createdBy` field definition
- `createdByOnCreateBeforeChangeHook` — sets `createdBy` on create
- `resolveUsersCollectionSlug(adminUser?)` — same resolution as the plugin (`admin.user` → slug)
...
8. On each **app collection** with `dataScope`: add an ownership field (`createdBy` or custom name) + create hook — then apply `getPermissionAccess` ([UTILS](https://github.com/zealamic/payload-plugin-rbac/blob/main/docs/UTILS.md))
...
| Ownership field (app CRUD) | Add `createdBy` relationship + hook per collection — [UTILS](https://github.com/zealamic/payload-plugin-rbac/blob/main/docs/UTILS.md#ownership-field-required-for-data-scope) |
| Custom auth users slug     | `admin.user` — plugin resolves `usersCollectionSlug` automatically                                              |
